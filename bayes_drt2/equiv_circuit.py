...
from .plotting import plot_nyquist, plot_bode, plot_eis
...
def plot_var_RC_fit_path(data, history, model=Z_var_num_RC, weighting='modulus', weight_split_character=0.5,
    ...
    for num, axrow in zip(num_RC, axes):
        ...
        # plot nyquist fit
        plot_nyquist(data, ax=axrow[1], unit_scale=unit_scale, c='k', alpha=0.7)
...
def plot_model(model, params, w=None, area=None, plot_type='all', plot_func='plot', axes=None, label='',
    ...
    if plot_type == 'nyquist':
        axes = plot_nyquist(data, area=area, ax=axes, label=label, plot_func=plot_func, unit_scale=unit_scale, c=c,
                            **kw)
    elif plot_type == 'bode':
        ...
        if axes is None:
            fig, axes = plt.subplots(1, 3, figsize=(12, 4))
        plot_eis(data, axes=axes, label=label, plot_func=plot_func, area=area, unit_scale=unit_scale, c=c, **kw)
    ...
    if mark_peaks:
        ...
        if plot_type == 'nyquist':
            plot_nyquist(peak_df, area=area, ax=axes, unit_scale=unit_scale, marker='x', s=50, c=c)
        elif plot_type == 'bode':
            plot_bode(peak_df, axes=axes, marker='x', s=50, area=area, c=c)
        elif plot_type == 'all':
            plot_nyquist(peak_df, area=area, ax=axes.ravel()[0], unit_scale=unit_scale, marker='x', s=50, c=c)
...
def mark_model_peaks(params, model, area=None, plot_type='all', axes=None, label='', marker='x', s=50, c='r',
    ...
    if plot_type == 'nyquist':
        axes = plot_nyquist(peak_df, area=area, ax=axes, label=label, unit_scale=unit_scale, marker=marker, s=s, c=c,
                            **kw)
    elif plot_type == 'bode':
        ...
        if axes is None:
            fig, axes = plt.subplots(1, 3, figsize=(12, 4))
        plot_nyquist(peak_df, area=area, ax=axes.ravel()[0], label=label, unit_scale=unit_scale, c='r', marker='x',
                     s=50)
...
def plot_fit(data, params, model, f_model=None, axes=None, unit_scale='auto', area=None, bode_cols=['Zmod', 'Zphz'],
    ...
    # Nyquist plot
    # ax.scatter(data['Zreal'],-data['Zimag'],s=6,label='Measured')
    plot_nyquist(data, area=area, ax=ax1, label='Measured', unit_scale=unit_scale, **data_kw)
    plot_nyquist(fit_df, area=area, ax=ax1, label='Fit', plot_func='plot', unit_scale=unit_scale, c=fit_color, **fit_kw)
    ...
    if mark_peaks:
        ...
        Z_peaks = model(f_peaks, **params)
        peak_df = construct_eis_df(f_peaks, Z_peaks)
        plot_nyquist(peak_df, area=area, ax=ax1, label='RQ Peak Frequencies', unit_scale=unit_scale, c=fit_color,
                     marker='x', s=50)
