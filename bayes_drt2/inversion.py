...
class Inverter:
	def __init__(self, basis_freq=None, basis_type='gaussian', epsilon=None, fit_inductance=True,
				 distributions={'DRT': {'kernel': 'DRT'}}):
		"""
		Parameters:
		-----------
		basis_freq: array (default: None)
			Frequencies to use for radial basis_type functions for all distributions. If None, determine from measurement frequencies.
		...
		basis_type: str (defualt: 'gaussian')
			Type of radial basis_type function to use. 'gaussian' is currently the only option.
		epsilon: float (default: None)
			Inverse length scale of radial basis_type functions. If None, determined automatically from basis_freq.
		...
		"""
		self._recalc_fit_matrices = True
		self._recalc_prediction_matrices = True
		self.distribution_matrices = {}
		self.basis_freq = basis_freq
		self.basis_type = basis_type
		self.epsilon = epsilon	# inverse length scale of RBFs
		self.fit_inductance = fit_inductance
		self.distributions = distributions
		self._cached_distributions = deepcopy(self.distributions)
		self.f_train = [0]
		self.Z_train = None
		self.f_predict = None
	...
	def ridge_fit(self, frequencies, Z, part='both',
		...
		# --------------------------
			...
			# get w_bar
			wbar = format_weights(target_scaled, hw_wbar, part)
...
"""
	...
	# L = construct_L(frequencies,tau=self.tau,basis_type=self.basis_type,epsilon=self.epsilon,order=reg_ord)
	# Lx2 = (L@coef)**2
	...
	def fit(self, frequencies, Z, part='both', scale_Z=True, nonneg=False, outliers=False, check_outliers=True,
		...
		if model_str is None:
			...
			model = CmdStanModel(
				stan_file=os.path.join(script_dir, 'stan_model_files', model_str),
				# exe_file=os.path.join(script_dir, 'stan_model_files', model_str.replace('.stan', '.exe'))
			)
		self.stan_model_name = model_str
		model_type = re.split('_|\.', model_str)[0]

		if model_type == 'Series-Parallel' and nonneg is False:
		...
		if mode == 'optimize':
			self.stan_mle = model.optimize(dat, iter=max_iter, seed=random_seed, inits=init, algorithm='lbfgs')
			self._opt_result = self.stan_mle.stan_variables()
		elif mode == 'sample':
	...
	def drift_map_fit(self, frequencies, Z, times, drift_model='x1', part='both', scale_Z=True, init_from_ridge=False,
		...
		if init_from_ridge:
			...
			if len(self.distributions) > 1:
				...
				init = self._get_init_from_ridge(frequencies, Z, 'optimize', nonneg=nonneg, outliers=outliers,
												 inductance_scale=inductance_scale, ridge_kw=ridge_kw)
				self._init_params = init
				dist_name = list(self.distributions.keys())[0]
				iv_ridge = init.copy()
		...
		# optimize posterior
		self._opt_result = model.optimizing(dat, iter=max_iter, seed=random_seed, inits=init)
	...
	def _get_stan_model(self, nonneg, outliers, drift, drift_model, fitY, SA):
		...
		# print(model_str)
		# model = load_pickle(os.path.join(script_dir, 'stan_model_files', model_str))
		model = CmdStanModel(stan_file=os.path.join(script_dir, 'stan_model_files', model_str + '.stan'))
							 # exe_file=os.path.join(script_dir, 'stan_model_files', model_str + '.exe'))
	...
	def _get_init_from_ridge(self, frequencies, Z, mode, nonneg, outliers, inductance_scale, ridge_kw):
		...
		if dist_type == 'series':
			...
			x_star = coef * self._Z_scale
		# Remove negatives if nonneg (very small negative values possible from cvxopt)
		if nonneg:
			x_star[x_star <= 0] = 1e-10
		
		init_values = {'x': x_star}
		...
		# if mode == 'optimize':
			...
			# ups_raw = q * 0.5 / 0.15
			# ups_raw[ups_raw < 1e-9] = 1e-9 # Set floor to avoid div by zero errors in d_ups 
			# init_values['ups_raw'] = ups_raw

		# input other parameters
		init_values['Rinf'] = self.R_inf / self._Z_scale
		if init_values['Rinf'] <= 0:
			init_values['Rinf'] = 1e-10	 # prevent negatives
		init_values['Rinf_raw'] = init_values['Rinf'] / 100
		init_values['induc'] = self.inductance / self._Z_scale
		if init_values['induc'] <= 0:
			init_values['induc'] = 1e-10  # prevent negatives
		init_values['induc_raw'] = init_values['induc'] / inductance_scale

		if outliers is True or outliers == 'auto':
			...
			if outliers is True or len(outlier_idx) > 0:
				...
				init_values['sigma_out_raw'] = sigma_out_raw
				init_values['sigma_out_scale'] = sigma_out_raw
	...
	# ===============================================
	def _prep_matrices(self, frequencies, Z, part, weights, dZ, scale_Z, penalty, fit_type, qphb_rp_scale=None,
					   sort_desc=True):
		...
		# Update tau and epsilon for each distribution
		for name, info in self.distributions.items():
			...
			if info.get('epsilon', self.epsilon) is None:
				# if neither dist-specific nor instance-level epsilon is specified
				dlnt = np.mean(np.diff(np.log(tau)))
				# temp_dist[name]['epsilon'] = (1 / dlnt)
				self.distributions[name]['epsilon'] = (1 / dlnt)
			elif info.get('epsilon', None) is None:
				# if dist-specific epsilon not specified, but instance-level epsilon is present
				# temp_dist[name]['epsilon'] = self.epsilon
				self.distributions[name]['epsilon'] = self.epsilon
			epsilon = self.distributions[name]['epsilon']  # temp_dist[name].get('epsilon', self.epsilon)
		...
		# set_distributions never gets called, and thus _recalc_fit_matrices does not get reset to True.
		if not check_equality(self.distributions, self._cached_distributions):
			self._recalc_fit_matrices = True
			self._recalc_prediction_matrices = True
			self.f_predict = None

		# store frequencies and Z
		...
		# else:
		#	  self.f_train = frequencies
		# # print(self._recalc_fit_matrices)
		# # print(freq_subset)

		# scale Z
		if scale_Z:
			Z = self._scale_Z(Z, fit_type, qphb_rp_scale)
		...
		weights = format_weights(Z, weights, part)
		W_re = np.diag(np.real(weights))
		W_im = np.diag(np.imag(weights))

		# If, after all status updates, fit matrices must be recalculated,
		# we must directly set self._f_train equal to frequencies.
		# Otherwise, f_train_subset_index will reflect the appropriate subset index for the previous matrices,
		# which will actually be replaced.
		# if self._recalc_fit_matrices:
		#	  self._f_train = frequencies
		#	  self._f_train_subset_index = []
		...
		for name, info in self.distributions.items():
			...
			#
			# # update distributions without overwriting self._recalc_fit_matrices
			...
			if self._recalc_fit_matrices or 'A_re' not in self.distribution_matrices[name].keys() or 'A_im' not in \
				...
				self.distribution_matrices[name]['A_re'] = construct_A(frequencies, 'real', tau=tau,
				...
				self.distribution_matrices[name]['A_im'] = construct_A(frequencies, 'imag', tau=tau,
																	   basis=self.basis_type,
																	   fit_inductance=self.fit_inductance,
																	   epsilon=epsilon,
																	   kernel=info['kernel'],
				...
				if dZ and info['kernel'] == 'DRT':
					# Z_re differentiation matrix (B@coef gives approx dZ'/dlnt at each basis_type tau)
					...
					B_pre = construct_A(1 / (2 * np.pi * B_tau), 'real', tau=tau, basis=self.basis_type,
										epsilon=epsilon,
				...
				if len(self._f_train_subset_index) > 0:
					# frequencies is a subset of f_train - use subset of rows
					A_re = self.distribution_matrices[name]['A_re'][self._f_train_subset_index, :].copy()
					A_im = self.distribution_matrices[name]['A_im'][self._f_train_subset_index, :].copy()
				else:
					# Nothing has changed - reuse same matrices
					A_re = self.distribution_matrices[name]['A_re'].copy()
					A_im = self.distribution_matrices[name]['A_im'].copy()
			...
			if penalty == 'integral':
				dist_mat[name]['M0'] = construct_M(1 / (2 * np.pi * tau), basis=self.basis_type, order=0,
												   epsilon=epsilon)
				dist_mat[name]['M1'] = construct_M(1 / (2 * np.pi * tau), basis=self.basis_type, order=1,
												   epsilon=epsilon)
				dist_mat[name]['M2'] = construct_M(1 / (2 * np.pi * tau), basis=self.basis_type, order=2,
												   epsilon=epsilon)

			elif penalty == 'discrete':
				dist_mat[name]['L0'] = construct_L(1 / (2 * np.pi * tau), tau=tau, basis=self.basis_type,
												   epsilon=epsilon,
												   order=0)
				dist_mat[name]['L1'] = construct_L(1 / (2 * np.pi * tau), tau=tau, basis=self.basis_type,
												   epsilon=epsilon,
												   order=1)
				dist_mat[name]['L2'] = construct_L(1 / (2 * np.pi * tau), tau=tau, basis=self.basis_type,
												   epsilon=epsilon,
												   order=2)

			elif penalty == 'cholesky':
				M0 = construct_M(1 / (2 * np.pi * tau), basis=self.basis_type, order=0, epsilon=epsilon)
				M1 = construct_M(1 / (2 * np.pi * tau), basis=self.basis_type, order=1, epsilon=epsilon)
				M2 = construct_M(1 / (2 * np.pi * tau), basis=self.basis_type, order=2, epsilon=epsilon)

				# scipy cholesky gives upper triangular by default, such that M = L.T@L.
				# Then x.T@M@x = x.T@L.T@L@x = ||L@x||^2
				dist_mat[name]['L0'] = cholesky(M0)
		...
		# Update recalc statuses and cached_distributions
		self._recalc_fit_matrices = False
		self._recalc_prediction_matrices = False  # Fit matrices can be used for prediction
		self._cached_distributions = deepcopy(self.distributions)
	...
	def _scale_Z(self, Z, fit_type, qphb_rp_scale=None):
		Zmod = (Z * Z.conjugate()) ** 0.5
		...
		if fit_type == 'qphb':
			...
			self._Z_scale = rp_est / qphb_rp_scale
		elif num_par == 1 and num_series == 0 and fit_type != 'ridge':
			Y = 1 / Z
	...
	def _get_prediction_matrices(self, frequencies, distributions):
		...
		# Create temporary dict to hold matrices
		pred_mat = {}
		for name in distributions:
			pred_mat[name] = {}

		# self._f_predict_subset_index = ('f_predict', np.array
		if self._recalc_prediction_matrices:
			...
			for name in distributions:
				...
				pred_mat[name]['A_re'] = construct_A(frequencies, 'real', tau=tau, basis=self.basis_type,
				...
				pred_mat[name]['A_im'] = construct_A(frequencies, 'imag', tau=tau, basis=self.basis_type,
													 fit_inductance=self.fit_inductance, epsilon=epsilon,
													 kernel=info['kernel'], dist_type=info['dist_type'],
													 symmetry=info.get('symmetry', ''), bc=info.get('bc', ''),
													 ct=info.get('ct', False), k_ct=info.get('k_ct', None)
													 )
				print('recalculated pred mat')
			# else:
			...
			# Existing matrices can be reused. Don't overwrite self.prediction_matrices
			if self._f_predict_eq_f_train:
				# frequencies are same as f_train. Reuse fit matrices
				for name in distributions:
					mat = self.distribution_matrices[name]
					pred_mat[name]['A_re'] = mat['A_re'].copy()
					pred_mat[name]['A_im'] = mat['A_im'].copy()
			elif self._f_predict_subset_index[0] == 'f_train':
				# frequencies is a subset of f_train. Use subset of rows
				for name in distributions:
					mat = self.distribution_matrices[name]
					pred_mat[name]['A_re'] = mat['A_re'][self._f_predict_subset_index[1], :].copy()
					pred_mat[name]['A_im'] = mat['A_im'][self._f_predict_subset_index[1], :].copy()
			elif self._f_predict_subset_index[0] == 'f_predict':
				# frequencies is a subset of f_predict. Use subset of rows
				for name in distributions:
					mat = self.prediction_matrices[name]
					pred_mat[name]['A_re'] = mat['A_re'][self._f_predict_subset_index[1], :].copy()
					pred_mat[name]['A_im'] = mat['A_im'][self._f_predict_subset_index[1], :].copy()
			else:
				# frequencies is same as f_predict. Reuse self.prediction_matrices
				for name in distributions:
					mat = self.prediction_matrices[name]
					pred_mat[name]['A_re'] = mat['A_re'].copy()
					pred_mat[name]['A_im'] = mat['A_im'].copy()
			# print('reused pred mat')

		self._recalc_prediction_matrices = False

		#	  # frequencies differ from f_predict
		#	  if np.min([rel_round(f, 10) in rel_round(self.f_predict, 10) for f in frequencies]) == True:
		#		  # if frequencies are a subset of f_predict, we can use submatrices of the existing A matrices
		#		  # instead of calculating new A matrices
		#		  f_index = np.array(
		#			  [np.where(rel_round(self.f_predict, 10) == rel_round(f, 10))[0][0] for f in frequencies])
		#
		#		  for name in distributions:
		#			  mat = self.prediction_matrices[name]
		#			  pred_mat[name]['A_re'] = mat['A_re'][f_index, :].copy()
		...
		#		  self.f_predict = frequencies
		# else:
		#	  # frequencies are same as f_predict. Use existing matrices
		#	  for name in distributions:
		#		  mat = self.prediction_matrices[name]
		#		  pred_mat[name]['A_re'] = mat['A_re'].copy()
		#		  pred_mat[name]['A_im'] = mat['A_im'].copy()
		...
		if percentile is not None:
			...
			if self.fit_type == 'map-drift':
				...
				for name, mat in pred_mat.items():
					...
					model_split = re.split('_|\.', self.stan_model_name)
					drift_str = [ms for ms in model_split if ms[:5] == 'drift'][0]
					drift_model = '-'.join(re.split('_|\.', drift_str)[1:])
	...
	def predict_sigma(self, frequencies, percentile=None, times=None):
		...
		if np.min(rel_round(self.f_train, 10) == rel_round(frequencies, 10)) == True:
			...
			if self.fit_type == 'qphb':
				sigma_re = self.qphb_params['sigma'].real
				sigma_im = self.qphb_params['sigma'].imag
			else:
				if self.fit_type == 'bayes' and percentile is not None:
					...
					raise ValueError('Error scale prediction is not available for ridge_fit')
	...
	def predict_distribution(self, name=None, tau=None, percentile=None, time=None):
		...
		# If eval_tau not specified, use basis_type tau
		if tau is None:
			tau = self.distributions[name]['tau']
		...
		if self.fit_type == 'map-drift':
			...
			if drift_model in ('x1', 'x2'):
				...
				phi = get_basis_func(self.basis_type)
				bases = np.array([phi(np.log(tau / t_m), epsilon) for t_m in basis_tau]).T
				F = bases @ x
				...
				F_rq = (1 / (2 * np.pi)) * np.sin((1 - phi_rq) * np.pi) / (
						np.cosh(phi_rq * np.log(tau / tau_rq)) - np.cos((1 - phi_rq) * np.pi))
	...
	def check_outliers(self, frequencies, Z, threshold, use_existing_fit, **ridge_kw):
		...
		if check_equality(frequencies, self.f_train) and check_equality(Z, self.Z_train) \
				and not self._recalc_fit_matrices and hasattr(self, 'distribution_fits'):
			fit_exists = True
		else:
			fit_exists = False
		...
		elif self.fit_type in ('map', 'bayes', 'qphb'):
	...
	def fit_peaks(self, distribution=None, eval_tau=None, percentile=None, time=None,
		...
		eval_tau : array, optional (default: None)
			tau grid over which to fit distribution. If None, go one decade beyond basis_type tau in each direction
		...
		# If eval_tau not given, go one decade beyond basis_type tau in each direction to capture all peaks
		if eval_tau is None:
		...
		F = self.predict_distribution(eval_tau, distribution, percentile, time)
	...
	def score_peak_fit(self, eval_tau=None, distribution=None, weights=None, percentile=None, time=None):
		...
		# If eval_tau not given, use basis_type tau
		if eval_tau is None:
	...
	def plot_distribution(self, ax=None, distribution=None, tau_plot=None, plot_bounds=True, plot_ci=True,
		...
		if len(self._f_train_subset_index) > 0:
			f_train = self.f_train[self._f_train_subset_index]
		else:
			f_train = self.f_train
		df = fl.construct_eis_df(f_train, self.Z_train)
	...
	def get_fit_attributes(self, which='all'):
		fit_attributes = {
			...
			'qphb': {'core': ['qphb_params'], 'detail': ['_iter_history']},
	...
	def load_fit_data(self, data):
		...
		f_pred_old = deepcopy(self.f_predict)
		self._cached_distributions = deepcopy(self.distributions)
	...
	def set_distributions(self, distributions):
		...
		distributions: dict
			...
			Each nested dict defines a distribution and should include the following keys:
				...
				basis_freq: array of frequencies to use as basis_type. If not specified, use self.basis_freq
				epsilon: epsilon value for basis_type functions. If not specified, use self.epsilon
		"""
	...
	def set_basis_freq(self, basis_freq):
		if hasattr(self, 'basis_freq'):
			if not check_equality(basis_freq, self.basis_freq):
				self._recalc_fit_matrices = True
				self._recalc_prediction_matrices = True
	...
	def set_basis_type(self, basis_type):
		check_basis_type(basis_type)
		if hasattr(self, 'basis_type'):
			if basis_type != self.basis_type:
				self._recalc_fit_matrices = True
				self._recalc_prediction_matrices = True
		self._basis_type = basis_type

	basis_type = property(get_basis_type, set_basis_type)
	...
	def set_f_train(self, frequencies):
		if self._recalc_fit_matrices or not hasattr(self, 'f_train'):
			# If fit matrices are going to be recalculated, must update f_train
			self._f_train = frequencies
			self._f_train_subset_index = []
			self._recalc_fit_matrices = True
		else:
			self._f_train_subset_index = []
			# Check if frequencies is the same as self.f_train
			if check_equality(rel_round(self._f_train, 10), rel_round(frequencies, 10)):
				self._f_train = frequencies
				# self._recalc_fit_matrices = False
			# Check if frequencies is a subset of self.f_train
			else:
				# If frequencies is a subset of self.f_train, we can use sub-matrices of the existing matrices
				# In this case, we should not update self._f_train
				if is_subset(frequencies, self._f_train):
					self._f_train_subset_index = np.array(
						[np.where(rel_round(self.f_train, 10) == rel_round(f, 10))[0][0] for f in frequencies]
					)
					# self._recalc_fit_matrices = False
				else:
					# if frequencies is not a subset of self.f_train, must recalculate matrices
					self._f_train = frequencies
					self._recalc_fit_matrices = True

	f_train = property(get_f_train, set_f_train)

	def get_f_predict(self):
		return self._f_predict

	def set_f_predict(self, frequencies):
		if self._recalc_prediction_matrices or not hasattr(self, 'f_predict'):
			# If prediction matrices must be recalculated, must overwrite self._f_predict
			...
			if check_equality(rel_round(self._f_train, 10), rel_round(frequencies, 10)):
				# self._f_predict = frequencies
				self._f_predict_eq_f_train = True
			# Check if frequencies is the same as self.f_predict
			elif check_equality(rel_round(self._f_predict, 10), rel_round(frequencies, 10)):
				self._f_predict = frequencies
			# Check if frequencies is a subset of self.f_train or self.f_predict
			else:
				# If frequencies is a subset of self.f_train or self.f_predict, we can use sub-matrices of the existing
				# matrices
				if is_subset(frequencies, self.f_predict):
				...
				elif is_subset(frequencies, self.f_train):
					# frequencies is a subset of f_train
					self._f_predict_subset_index = ('f_train', np.array(
						[np.where(rel_round(self.f_train, 10) == rel_round(f, 10))[0][0] for f in frequencies]
					)
													)
					# self._recalc_prediction_matrices = False
	...
	def set_fit_inductance(self, fit_inductance):
		...
		if self._recalc_fit_matrices == False and hasattr(self, 'A_im'):
			self.A_im[:, 1] = -2 * np.pi * self.f_train

	fit_inductance = property(get_fit_inductance, set_fit_inductance)

	# def check_fit_recalc_status(self):
	#	  """
	#	  Perform a final check on matrix recalculation status
	#	  """
	#	  if self._recalc_fit_matrices:
	#		  self._f_train_subset_index = []
	#		  self._f_train
...
# TODO: implement re-im error variance correlation


def format_weights(Z, weights, part):
	"""
	Format real and imaginary weight vectors
	Parameters:
	-----------
	weights : str or array (default: None)
		Weights for fit. Standard weighting schemes can be specified by passing 'uniform', 'modulus', or 'proportional'.
		Custom weights can be passed as an array. If the array elements are real, the weights are applied to both the real and imaginary parts of the impedance.
		If the array elements are complex, the real parts are used to weight the real impedance, and the imaginary parts are used to weight the imaginary impedance.
		If None, all points are weighted equally.
	...
	if weights is None:
		...
		if weights == 'modulus':
			...
			raise ValueError(
				f"Invalid weights argument {weights}. String options are 'uniform', 'modulus', 'proportional', and 'prop_adj'")
	elif type(weights) in (float, int):
		# assign constant value
		weights = np.ones_like(Z) * (1 + 1j) * weights
	elif type(weights) in (complex, np.complex128):
		# assign constant value
		weights = np.ones_like(Z) * weights
	elif len(weights) != len(Z):
	...
	if part == 'both':
		...
		weights = np.real(weights) + 1j * np.ones_like(Z)
	elif part == 'imag':
		if np.min(np.isreal(weights)) == True:
			# if weights are real, apply to imag
			weights = np.ones_like(Z) + 1j * weights
		else:
			# if weights are complex, leave them
			pass
	else:
