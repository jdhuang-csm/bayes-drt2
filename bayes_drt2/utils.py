...
# Functions for pickling objects
# ------------------------------
...
def is_subset(x, s):
    """
    Return True if x is a subset of s, else return False
    :param x:
    :param s:
    :return:
    """
    if s is None:
        ...
        return np.min([rel_round(xi, 10) in rel_round(s, 10) for xi in x])
...
def get_ppd(frequencies):
    lf_max = np.log10(np.max(frequencies))
    lf_min = np.log10(np.min(frequencies))
    num_decades = lf_max - lf_min

    return (len(frequencies) - 1) / num_decades
...
def check_basis_type(basis_type):
    options = ['gaussian', 'Cole-Cole', 'step']
    if basis_type not in options:
        raise ValueError(f'Invalid basis_type {basis_type}. Options: {options}')


def check_penalty(penalty):
    options = ['discrete', 'integrated', None]
    if penalty not in options:
        raise ValueError(f'Invalid penalty {penalty}. Options: {options}')
