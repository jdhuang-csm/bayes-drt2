...
def get_basis_func(basis):
    ...
    if basis == 'gaussian':
        ...
        raise ValueError(f'Invalid basis_type {basis}. Options are gaussian')
    return phi
...
def get_A_func(part, basis='gaussian', kernel='DRT', dist_type='series', symmetry='planar', bc=None, ct=False,
	...
	basis_type : string, optional (default: 'gaussian')
		basis_type function
	"""
...
def construct_A(frequencies, part, tau=None, basis='gaussian', fit_inductance=False, epsilon=1,
	...
	tau : array, optional (default: None)
		Time constants at which to center basis_type functions. If None, use time constants
	...
	epsilon : float, optional (default: 1)
		Shape parameter for chosen basis_type function
	"""
...
def get_M_func(basis='gaussian', order=1):
    ...
    if order == 0:
        if basis == 'gaussian':
            ...
            raise ValueError(f'Invalid basis_type {basis}')
    elif order == 1:
        if basis == 'gaussian':
            def func(w_n, t_m, epsilon):
                a = epsilon * np.log(1 / (w_n * t_m))
...
def construct_M(frequencies, basis='gaussian', order=1, epsilon=1):
	...
	-----------
	frequencies : array
		Frequencies at which basis_type functions are centered
	basis_type : string, optional (default: 'gaussian')
		Basis function used to approximate DRT
	order : int, optional (default: 1)
    ...
    if is_loguniform(frequencies):
        # only need to calculate 1st column (symmetric toeplitz matrix) IF gaussian - may not apply to other basis_type functions
        # w_0 = omega[0]
        t_0 = 1 / omega[0]
